...
function AddForm() {
    ...
    const URL_ADD_ADMIN = "/user/adminsignup "
    const URL_ADD_USER = "/user/signup "
    const URL_ADD_EVENTPROVIDER = "/user/eventprovidersignup "
    ...
    const [eventCompany, setEventCompany] = React.useState('');
    const [isEventProvide, setIsEventProvide] = React.useState(false)
    ...
    const handleSubmitForSimple = async (e) => {
        e.preventDefault();
        ...
        try {
            ...
            if (file[0]) {
                ...
                const resposeUpload = axios.post(UPLOAD_IMAGE, data, { Headers: { 'Content-Type': 'multipart/form-dat' }, withCredentials: true }).then((res) => {
                    const response = axios.post(URL_ADD_USER, {
    ...
    const handleSubmitForAdmin = async (e) => {
        ...
        try {
            if (file[0]) {
                ...
                data.append('file', file[0])
                const resposeUpload = axios.post(UPLOAD_IMAGE, data, { Headers: { 'Content-Type': 'multipart/form-dat' }, withCredentials: true }).then((res) => {
                    const response = axios.post(URL_ADD_ADMIN, {
                        "name": name, "email": email,
                        "surname": surName, "password": pass, "phoneNumber": phone, "birthDate": birth, "image": res.data.filename
                    }, { Headers: { 'Content-Type': 'applocation/json' }, withCredentials: true })
                })
    ...
    const handleSubmitForEventProvider = async (e) => {
        e.preventDefault();
        ...
        try {
            if (file[0]) {
                ...
                data.append('file', file[0])
                const resposeUpload = axios.post(UPLOAD_IMAGE, data, { Headers: { 'Content-Type': 'multipart/form-dat' }, withCredentials: true }).then((res) => {
                    const response = axios.post(URL_ADD_EVENTPROVIDER, {
                        "name": name, "email": email,
                        "surname": surName, "password": pass, "phoneNumber": phone, "birthDate": birth, "image": res.data.filename, "eventComapny": eventCompany
                    }, { Headers: { 'Content-Type': 'applocation/json' }, withCredentials: true })
                })
            }
            else {
    ...
    const handleSubmit = async (e) => {
           e.preventDefault();
            switch(role){
                case 'simpleUser':{
                    handleSubmitForSimple(e)
                }break;
                case 'admin':{
                    handleSubmitForAdmin(e)
                }break;
                case'eventProvider':{
                    handleSubmitForEventProvider(e)
                }break;
            }
            setName('')
            setSurName('')
    ...
    const handleEventProviderchange = (event) => {
        if (event.target.value === 'eventProvider') {
            setIsEventProvide(true)
        } else {
            setIsEventProvide(false)
        }
    }
    const handleChange = (event) => {
        setRole(event.target.value);
        console.log(event.target.value)
        handleEventProviderchange(event)
    };
        ...
        <>
            ...
            >
                <Grid container xs={12} sx={{ padding: '3%' }}>
                    <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'center' }}> <Typography sx={{ fontSize: '40px', marginBottom: '2%' }}>  Add User</Typography> </Grid>
                    ...
                    <Grid item xs={12}>
                        <form onSubmit={handleSubmit}>
                            <Grid container xs={12}>
                                ...
                                {
                                    isEventProvide ?
                                        (<Grid item sx={{ display: 'flex', justifyContent: 'flex-start', marginBottom: '2%' }} xs={12}>
                                            <TextField sx={{ width: '20em' }} id="EventCompany" onChange={(newValue) => { setEventCompany(newValue.target.value) }} label="Event company Name" variant="outlined" />
                                        </Grid>) : (null)
                                }
